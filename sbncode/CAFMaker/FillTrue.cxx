...
namespace caf {
  ...
  void FillTrackCaloTruth(const std::map<int, std::vector<std::pair<geo::WireID, const sim::IDE*>>> &id_to_ide_map,
    ...
    for (auto const &ide_pair: match_ides) {
      chan_2_ides[channelMap.PlaneWireToChannel(ide_pair.first)].push_back(ide_pair.second);
    }
    ...
    for (unsigned iplane = 0; iplane < 3; iplane++) {
      for (caf::SRCaloPoint &p: srtrack.calo[iplane].points) {
        ...
        if (closest_dist >= 0. && direction.Mag() > 1e-4) {
          ...
          geo::PlaneGeo const& plane = channelMap.Plane(planeid);
          float angletovert = channelMap.WireAngleToVertical(plane.View(), planeid) - 0.5*::util::pi<>();
          ...
          // Map to wires
          if (sce && sce->EnableSimSpatialSCE()) { 
            int corr = geom.TPC(plane.ID()).DriftDir().X();
 ...
 void FillSliceFakeReco(const std::vector<art::Ptr<recob::Hit>> &hits,
  ...
  std::map<int, std::vector<std::pair<geo::WireID, const sim::IDE*>>> PrepSimChannels(const std::vector<art::Ptr<sim::SimChannel>> &simchannels, const geo::WireReadoutGeom &wireReadoutGeom) {
    ...
    for (const art::Ptr<sim::SimChannel> sc : simchannels) {
      ...
      std::vector<geo::WireID> maybewire = wireReadoutGeom.ChannelToWire(channel);
