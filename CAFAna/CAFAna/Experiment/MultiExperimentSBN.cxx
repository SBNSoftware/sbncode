...
namespace ana
{
                      ...
                      const std::vector<std::pair<const ISyst*, const ISyst*>>& corrs)
  {
    ...
    for(auto it: corrs){
      ...
      if(already.find(it.second) == already.end()){
        ...
        std::cout << "MultiExperimentSBN::SetSystCorrelations(): Warning!\n"
  ...
  //----------------------------------------------------------------------
  void MultiExperimentSBN::SaveTo(TDirectory* dir) const
  {
    bool hasCorr = false;
    for(auto it: fSystCorrelations) if(!it.empty()) hasCorr = true;

    if(hasCorr){
      std::cerr << "Warning in MultiExperimentSBN: systematic correlations are set and will not be serialized by this call to SaveTo(). You will have to re-set them once you load the experiment back in." << std::endl;
    }

    TDirectory* tmp = dir;

    dir->cd();
    TObjString("MultiExperimentSBN").Write("type");
  ...
  //----------------------------------------------------------------------
  std::unique_ptr<MultiExperimentSBN> MultiExperimentSBN::LoadFrom(TDirectory* dir)
  {
    TObjString* ptag = (TObjString*)dir->Get("type");
    assert(ptag);
    assert(ptag->GetString() == "MultiExperimentSBN");
    ...
    return std::unique_ptr<MultiExperimentSBN>(new MultiExperimentSBN(expts));
  }
}
