...
void ProcessorBase::BuildEventTree(gallery::Event& ev) {
  ...
  for (size_t i=0; i<mctruths.size(); i++) {
    ...
    if (!wghs.empty()) {
      ...
      //       vector in the "weightmap" below. The order in which weights from different producers are combined
      ...
      for (auto const& wgh : wghs) {
        for (const std::pair<std::string, std::vector<double>> &this_wgh: wgh->at(i).fWeight) {
          ...
          if (interaction.weightmap.count(this_wgh.first) == 0) {
            interaction.weightmap.insert(this_wgh);
          }
          // If we have seen the name before, append this instance to the last one
          else {
            interaction.weightmap.at(this_wgh.first).insert(interaction.weightmap.at(this_wgh.first).end(), this_wgh.second.begin(), this_wgh.second.end());
          }
        }
