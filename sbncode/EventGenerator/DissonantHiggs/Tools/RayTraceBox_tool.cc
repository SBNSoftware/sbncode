...
bool RayTraceBox::IntersectDetector(HiggsFlux &flux, std::array<TVector3, 2> &intersection, double &weight) {
  ...
  if ((flux.pos.Vect() - A).Mag() < (A-B).Mag() && (flux.pos.Vect() - B).Mag() < (A-B).Mag()) {
    throw cet::exception("RayTraceBox Exception", "Input higgs flux starts inside detector volume: "
