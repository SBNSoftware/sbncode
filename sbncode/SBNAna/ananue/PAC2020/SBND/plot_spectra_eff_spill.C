...
void plot_spectra_eff_spill(std::string name = "nue")
{
  ...
  texEff.open("tex/" + name + "_spill_Eff.tex");
  texEffRel.open("tex/" + name + "_spill_EffRel.tex");
  texNum.open("tex/" + name + "_spill_Num.tex");
  texHdr.open("tex/" + name + "_spill_Hdr.tex");
  ...
  std::vector<std::vector<Spectrum*>> baseSpecs;
  for (unsigned int lType = 0; lType < kNType; lType++) {
    std::vector<Spectrum*> theseBaseSpecs;
    for (unsigned int iVar = 0; iVar < kNVar; ++iVar) {
      std::string thissuffix = plots[iVar].suffix + "_" + sels[0].suffix + "_" + types[lType].suffix;
      ...
      theseBaseSpecs.push_back(spec);
    }
    baseSpecs.push_back(theseBaseSpecs);
  }
  ...
  TFile* newFile = new TFile(("plots/plots_eff_spill_" + name + ".root").c_str(), "RECREATE");
  ...
  for (unsigned int iVar = 0; iVar < kNVar; ++iVar) {
    ...
    for (unsigned int jSel = 0; jSel < kNSel; ++jSel) {
      ...
      if (iVar == 0) {
        ...
        thisTexEff.open("tex/cuts_" + name + "_" + sels[jSel].suffix + "_spill_Eff.tex");
        thisTexEffRel.open("tex/cuts_" + name + "_" + sels[jSel].suffix + "_spill_EffRel.tex");
        thisTexNum.open("tex/cuts_" + name + "_" + sels[jSel].suffix + "_spill_Num.tex");
      ...
      c->Print(("plots/spill_" + plots[iVar].suffix + "_" + sels[jSel].suffix + "_" + name + ".png").c_str());
