...
//namespace sbn {
  ...
  //float getFOM2( const ub_BeamHeader& bh, const std::vector<ub_BeamData>& bd, const bnb::bnbAutoTune settings, bool useAutoTune)
  {
    ...
    double fom=0;
    //TFile* f = TFile::Open("/exp/sbnd/app/users/maxd/BNBQualityCut/ubooneData/pnfs/pnfs/uboone/persistent/uboonebeam/beam_data/bnb_data/00/01/47/68/beamdata_00014768.root", "READ");
    TFile* f = TFile::Open("/exp/sbnd/app/users/maxd/BNBQualityCut/ubooneData/pnfs/uboone/persistent/uboonebeam/beam_data/bnb_data/00/02/57/44/beamdata_00025744.root", "READ");

    if (!f || f->IsZombie()) {
      std::cerr << "Error opening file.root\n";
      return;
    }

    // Get the TTree named "bnb"
    TTree* tree = (TTree*)f->Get("bnb");
    if (!tree) {
      std::cerr << "TTree 'bnb' not found in file.root\n";
      return;
    }

    std::map<int, double> hp875_offset_map;
    std::map<int, double> vp875_offset_map;
    std::map<int, double> hptg1_offset_map;
    std::map<int, double> hptg2_offset_map;
    std::map<int, double> vptg1_offset_map;
    std::map<int, double> vptg2_offset_map;   
    
    //hp875_offset = hp875_offset_map[];
    

    //Other run 5 autotune offsets -3.4, 1.48, -1., -1., 1.07, -3.15
    // double hp875_offset= -3.4;
    // double vp875_offset= 1.48;
    // double hptg1_offset= -1.0;
    // double vptg1_offset= -1.0;
    // //double hptg2_offset= -1.0;
    // //double vptg2_offset= -1.0;
    // double hptg2_offset= 1.07;
    // double vptg2_offset= -3.15; 





    //Run 5 autotune values
    // double hp875_offset= -1.2;
    // double vp875_offset= 1.3;
    // double hptg1_offset= -1.0;
    // double vptg1_offset= -1.0;
    // //double hptg2_offset= -1.0;
    // //double vptg2_offset= -1.0;
    // double hptg2_offset= -0.4;
    // double vptg2_offset= 0.1; 



    //From IFBeam for Run 25744
    // double hp875_offset= -1.3359030485153198;
    // double vp875_offset= 1.3375698328018188;
    // double hptg1_offset= -0.1474824994802475;
    // double vptg1_offset= -0.24208535254001617;
    // //double hptg2_offset= -1.0;
    // //double vptg2_offset= -1.0;
    // double hptg2_offset= -0.5022110939025879;
    // double vptg2_offset= 0.12833189964294434; 
    ...
    // double hp875_offset= -2.2;
    // double vp875_offset= +1.7;
    // double hptg1_offset= -0.2;
    // double vptg1_offset= +0.38;
    // //double hptg2_offset= -1.0;
    // //double vptg2_offset= -1.0;
    // double hptg2_offset= +0.5;
    // double vptg2_offset= +1.0;
    ...
    //std::vector<double> mw875;// = nullptr; 
    //std::vector<double> mw876;// = nullptr;
    //std::vector<double> mwtgt;// = nullptr;
    double mw875[96];
    double mw876[96];
    double mwtgt[96];
    
    tree->SetBranchAddress("tor860", &tor860);
    tree->SetBranchAddress("tor875", &tor875);
    tree->SetBranchAddress("hp875", &hp875);
    tree->SetBranchAddress("vp875", &vp875);
    tree->SetBranchAddress("hptg1", &hptg1);
    tree->SetBranchAddress("vptg1", &vptg1);
    tree->SetBranchAddress("hptg2", &hptg2);
    tree->SetBranchAddress("vptg2", &vptg2);
    tree->SetBranchAddress("fom2", &fom2);
    tree->SetBranchAddress("m875bb", &mw875);    
    tree->SetBranchAddress("m876bb", &mw876);    
    tree->SetBranchAddress("mmbtbb", &mwtgt);
    ...
    */
    ...
    TCanvas* c1 = new TCanvas("c1", "c1", 800,600);
    TH1D* calcFOMhist = new TH1D("CalcFOMHist", "Calculated FOM Histogram; FOM; Entries", 100, 0.0 , 5.0);
    TCanvas* c2 = new TCanvas("c2", "c2", 800,600);
    //TH2D* FOMvfom2 = new TH2D("FOMvfom2", "FOM vs fom2; FOM; fom2", 100, 0. , 1.0, 100, 0., 1.0);
    TH2D* FOMvfom2 = new TH2D("FOMvfom2", "FOM vs fom2; FOM; fom2", 100, 0.95 , 1.0, 100, 0.95, 1.0);
    Long64_t firstTS=0; Long64_t lastTS =0;
    
    Long64_t nentries = tree->GetEntries();
    for (Long64_t i = 0; i < nentries; ++i) {
      tree->GetEntry(i);
      //std::cout << "Entry " << i << ": fom2 = " << fom2 << "  | " << mw876.size() << std::endl;;
      double tor = tor860;
      if (i==0) firstTS = tstamp;
      if (i == nentries-1) lastTS = tstamp;
      fom=0;
      if ( hptg1 <0.0){
	fom =-2;
        // hp875_offset = bad_hp875_offset;
	// vp875_offset =  bad_vp875_offset;
	// hptg1_offset = bad_hptg1_offset;
	// vptg1_offset = bad_vptg1_offset;
	// hptg2_offset = bad_hptg2_offset;
	// vptg2_offset = bad_vptg2_offset;
      }
	...
	if (hptg2 != -999 && hp875!=-999) {
	  horang=((hptg2-hptg2_offset)-(hp875-hp875_offset))/(hptg2_zpos-hp875_zpos);
	  horpos=(hp875-hp875_offset)+horang*(target_center_zpos-hp875_zpos);
	 } else if (hptg1!=-999 && hp875 !=-999) {
	   horang=((hptg1-hptg1_offset)-(hp875-hp875_offset))/(hptg1_zpos-hp875_zpos);
	   horpos=(hp875-hp875_offset)+horang*(target_center_zpos-hp875_zpos);
	 } else {
	  //missing horizontal BPM data
	  fom=-1;
	...
	if (vptg2!=-999 && vp875!=-999) {
	  verang=((vptg2-vptg2_offset)-(vp875-vp875_offset))/(vptg2_zpos-vp875_zpos);
	  verpos=(vp875-vp875_offset)+verang*(target_center_zpos-vp875_zpos);
	} else if (vptg1!=-999 && vp875!=-999) {
	   verang=((vptg1-vptg1_offset)-(vp875-vp875_offset))/(vptg1_zpos-vp875_zpos);
	   verpos=(vp875-vp875_offset)+verang*(target_center_zpos-vp875_zpos);
	} else {
	  fom=-1;
	}
	...
	fom=4;
      ...
      if(fom==0) fom=1-pow(10,sbn::calcFOM(horpos,horang,verpos,verang,tor,tgtsx,tgtsy));
      
      
      //std::cout <<tstamp <<  "FOM: " <<fom << "  fom2: " << fom2 << std::endl;

      calcFOMhist->Fill(fom);
      FOMvfom2->Fill(fom, fom2);

      //if ( fom-fom2 > 0.01) {
      if (fom !=0 && fom!=4 && fom - fom2 < 0.002 && fom-fom2 > -0.002) {
	std::cout <<fom << "  " << fom2 << "  " <<  hp875 << "  " << vp875 << "  " << hptg1 << "  " << vptg1 << "  " << hptg2 << "  " << vptg2 << std::endl; 
      }
      
    }    
    
    std::cout << firstTS << " |  " << lastTS << std::endl; 

    c1->cd();
    calcFOMhist->Draw("HIST");
