...
public:
  ...
  recob::Hit MakeHit(const recob::Wire &wire, const geo::WireID &wireID, const geo::GeometryCore *geo, const geo::WireReadoutGeom* wireReadoutGeom, raw::TDCtick_t startTick, raw::TDCtick_t endTick) const;
  recob::Hit MakeHit(const raw::RawDigit &digit, const geo::WireID &wireID, const geo::GeometryCore *geo, const geo::WireReadoutGeom* wireReadoutGeom, raw::TDCtick_t startTick, raw::TDCtick_t endTick) const;
...
recob::Hit sbn::TrackAreaHit::MakeHit(const recob::Wire &wire, const geo::WireID &wireID, const geo::GeometryCore *geo, const geo::WireReadoutGeom *wireReadoutGeom, raw::TDCtick_t startTick, raw::TDCtick_t endTick) const {
  // garbage values for some fields which are not applicable
  raw::ChannelID_t hitChannel = wireReadoutGeom->PlaneWireToChannel(wireID);
  ...
  short int hitNDF = 0;
  geo::View_t hitView = wireReadoutGeom->Plane(wireID).View();
  geo::SigType_t hitSignalType = wireReadoutGeom->SignalType(wireID);
...
recob::Hit sbn::TrackAreaHit::MakeHit(const raw::RawDigit &digit, const geo::WireID &wireID, const geo::GeometryCore *geo, const geo::WireReadoutGeom* wireReadoutGeom, raw::TDCtick_t startTick, raw::TDCtick_t endTick) const {
...
void sbn::TrackAreaHit::produce(art::Event& e)
{
  ...
  for (unsigned i = 0; i < tracks.size(); i++) {
    ...
    for (auto const &plane: wireReadoutGeom->Iterate<geo::PlaneGeo>(cid)) {
      ...
      for (size_t j = 0; j < track.NumberTrajectoryPoints()-1; j++) {
        ...
        double angleToVert = wireReadoutGeom->WireAngleToVertical(plane.View(), plane.ID()) - 0.5*::util::pi<>();
        double cosgamma = std::abs(std::sin(angleToVert)*dir.Y() + std::cos(angleToVert)*dir.Z()); 

        geo::PlaneGeo const& first_plane = wireReadoutGeom->Plane({tpc.ID(), 0});
        ...
        for (int wire = wireStart; wire != wireEnd; wire += incl) {
          ...
          unsigned channel = wireReadoutGeom->PlaneWireToChannel(wireID);
          ...
          if (fUseWires) {
            ...
            // make the hit
            thisHit = MakeHit(*wires.at(wire_ind), wireID, geo, wireReadoutGeom, startTick, endTick);
          }
          else {
            // Find the digits
            int digit_ind = -1;
            ...
            // Make the hit
            thisHit = MakeHit(*digits.at(digit_ind), wireID, geo, wireReadoutGeom, startTick, endTick);
          }
