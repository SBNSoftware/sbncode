...
namespace single_photon
{
  ...
  void AnalyzeShowers(
    ...
    for (ShowerVector::const_iterator iter = showers.begin(), iterEnd = showers.end(); iter != iterEnd; ++iter)
    {
      ...
      for (geo::PlaneGeo const& plane: paras.s_wireReadoutGeom->Iterate<geo::PlaneGeo>()) {
        //6 planes in SBND
        //WireAngleToVertical  : 30 ,150,90,150,30 ,90
        //ub wire angles    : 30 ,150,90  (respected to beam,z)
        //Pitch        : 0.3,0.3,0.3,0.3,0.3,0.3

        const double angToVert(paras.s_wireReadoutGeom->WireAngleToVertical(plane.View(), plane.ID())+0.5*M_PI);//wire angle respected to z + pi/2
