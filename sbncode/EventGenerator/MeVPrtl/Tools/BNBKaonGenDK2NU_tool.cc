...
//   fFluxMode = pset.get<std::string>("FluxMode", "G4BNB"); 
//   if (fFluxMode != "BooNE" && fFluxMode != "Dk2Nu") {
//     throw cet::exception("BNBKaonGen")
//       << "Invalid FluxMode = " << fFluxMode
//       << " (allowed: BooNE, Dk2Nu)";
//   }
...
// const bsim::Dk2Nu *BNBKaonGen::GetNextEntry() {
//   // new file -- set the start entry 
//   if (fNewFile) {
//     // wrap file index around
//     if (fFileIndex >= fFluxFiles.size()) {
...
//   if (fFluxMode == "G4BNB") {
//     mf::LogWarning("BNBKaonGen") << "BNBKaonGen is set to G4BNB mode, but this tool only supports BooNE mode. Switching to BooNE mode.";
...
// simb::MCFlux BNBKaonGen::MakeMCFlux(const bsim::Dk2Nu &dk2nu) {
//   simb::MCFlux flux;
...
/**
 *  @brief  BNBKaonGenDK2NU class definition
 */
class BNBKaonGenDK2NU : public IMesonGen
{
public:
    BNBKaonGenDK2NU(fhicl::ParameterSet const &pset);
    ~BNBKaonGenDK2NU();
...
private:
  ...
  std::string fTreeName;
  std::string fMetaTreeName;
  std::string fFluxMode;               // BooNE or Dk2Nu
  ...
  bool fBranchesInitialized = false;
...
BNBKaonGenDK2NU::BNBKaonGenDK2NU(fhicl::ParameterSet const &pset):
  IMeVPrtlStage("BNBKaonGenDK2NU")
{
  configure(pset);
...
BNBKaonGenDK2NU::~BNBKaonGenDK2NU()
{
  if (fDk2Nu) delete fDk2Nu;
  if (fBooNe) delete fBooNe;
}
...
void BNBKaonGenDK2NU::configure(fhicl::ParameterSet const &pset)
{
  fSearchPath       = pset.get<std::string>("SearchPath");
  fSearchPatterns   = pset.get<std::vector<std::string>>("FluxFiles");
  fMaxFluxFileMB    = pset.get<unsigned long>("MaxFluxFileMB", 2 * 1024);
  ...
  // Read flux mode from FHiCL
  fFluxMode = pset.get<std::string>("FluxMode", "BooNE");

  // Normalize user input
  if      (fFluxMode == "boone") fFluxMode = "BooNE";
  else if (fFluxMode == "dk2nu") fFluxMode = "Dk2Nu";

  // Validate mode
  if (fFluxMode != "BooNE" && fFluxMode != "Dk2Nu") {
    throw cet::exception("BNBKaonGenDK2NU")
      << "Invalid FluxMode = " << fFluxMode
      << " (allowed values: BooNE, Dk2Nu)";
  }

  // Log confirmation
  mf::LogInfo("BNBKaonGenDK2NU")
    << "============================================\n"
...
// ============================================================================
// Initialize ROOT branches depending on FluxMode
// ============================================================================

void BNBKaonGenDK2NU::InitBranches()
{
  if (fBranchesInitialized) return;

  if (!fFluxTree) {
    throw cet::exception("BNBKaonGenDK2NU") << "Flux tree is not initialized";
  }

  if (fFluxMode == "Dk2Nu") {
    // Bind dk2nu branch only in Dk2Nu mode
    fFluxTree->SetBranchAddress("dk2nu", &fDk2Nu);
    mf::LogInfo("BNBKaonGenDK2NU") << "Initialized dk2nu branch";
  }
  else {
    // BooNE reader manages its own file access
    mf::LogInfo("BNBKaonGenDK2NU") << "Using BooNE reader (no ROOT branch binding)";
  }

  fBranchesInitialized = true;
}
...
std::vector<std::string> BNBKaonGenDK2NU::LoadFluxFiles()
{
  art::ServiceHandle<IFDH> ifdhp;

  std::vector<std::pair<std::string, long>> allFiles;

  // Find all matching flux files
  for (unsigned i = 0; i < fSearchPatterns.size(); i++) {
    auto thisList = ifdhp->findMatchingFiles(fSearchPath, fSearchPatterns[i]);
    std::copy(thisList.begin(), thisList.end(), std::back_inserter(allFiles));
  }
  ...
  // Copy files locally
  auto localFiles = ifdhp->fetchSharedFiles(selected, fFluxCopyMethod);
...
// ============================================================================
// Load POT from metadata tree
// ============================================================================

double BNBKaonGenDK2NU::LoadPOT()
{
  TTreeReader metaReader(fMetaTreeName.c_str(), fFluxFile);
  TTreeReaderValue<double> pot(metaReader, "pots");
...
// ============================================================================
// Return accumulated POT
// ============================================================================

double BNBKaonGenDK2NU::GetPOT()
{
  double ret = fAccumulatedPOT;
  fAccumulatedPOT = 0.;
  return ret;
...
// ============================================================================
// Unified GetNextEntry
// ============================================================================

void BNBKaonGenDK2NU::GetNextEntry()
{
  ...
  if (fNewFile) {
    ...
    if (!fFluxTree) {
      throw cet::exception("BNBKaonGenDK2NU")
        << "Could not load tree " << fTreeName;
    }

    // Initialize branches depending on mode
    InitBranches();

    // BooNE reader must be re-created per file
    if (fFluxMode == "BooNE") {
      if (fBooNe) delete fBooNe;
      fBooNe = new bsim::BooNe(fFluxFiles[fFileIndex].c_str());
    }

    // Start at a random entry in this file
    fEntryStart = CLHEP::RandFlat::shootInt(
      fEngine, fFluxTree->GetEntries() - 1);

    fEntry = fEntryStart;

    // Load POT only for Dk2Nu mode
    if (fFluxMode == "Dk2Nu") {
      fThisFilePOT = LoadPOT();
    }
    ...
    // If last entry was reached, move to next file
    if ((fEntry + 1) % fFluxTree->GetEntries() == fEntryStart) {
      fFileIndex++;
      fNewFile = true;
    }
  ...
  // Read current entry from ROOT tree
  fFluxTree->GetEntry(fEntry);
  ...
  if (fFluxMode == "BooNE") {
    ...
    // Accumulate POT from BooNE
    fAccumulatedPOT += fBooNe->GetPOT();
  }
  else {
    // Dk2Nu POT is evenly distributed per entry
    fAccumulatedPOT += fThisFilePOT / fFluxTree->GetEntries();
  }
}

// ============================================================================
// Build MCFlux according to FluxMode
// ============================================================================

simb::MCFlux BNBKaonGenDK2NU::BuildMCFlux()
{
  if (fFluxMode == "BooNE") {
    return MakeMCFlux(*fBooNe);
  }
  else {
    return MakeMCFlux(*fDk2Nu);
  }
}

// ============================================================================
// GetNext (public interface)
// ============================================================================

simb::MCFlux BNBKaonGenDK2NU::GetNext()
{
  GetNextEntry();
  return BuildMCFlux();
}
...
// ============================================================================
// MakeMCFlux overloads 
// ============================================================================

// --- BooNE version ---
simb::MCFlux BNBKaonGenDK2NU::MakeMCFlux(const bsim::BooNe &boone)
{
  simb::MCFlux flux;
  auto fBooneNtp = boone.myNtuple;
...
// --- Dk2Nu version ---
simb::MCFlux BNBKaonGenDK2NU::MakeMCFlux(const bsim::Dk2Nu &dk2nu)
{
  simb::MCFlux flux;
...
DEFINE_ART_CLASS_TOOL(BNBKaonGenDK2NU)
