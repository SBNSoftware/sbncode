...
private:
  ...
  geo::Point_t PositionAbsolute(const geo::Point_t &p, const geo::GeometryCore& geom, const geo::WireReadoutGeom &channelMap, const spacecharge::SpaceCharge *sce);
...
std::array<float, 2> HitVector(const recob::Hit &hit, const geo::WireReadoutGeom &channelMap, const detinfo::DetectorPropertiesData &dprop) {
  float wire_distance = hit.WireID().Wire;
  // convert to cm
  float wire_distance_cm = wire_distance * channelMap.Plane(geo::PlaneID::first()).WirePitch();
...
std::array<float, 2> VertexVector(const recob::Vertex &vert, const geo::PlaneID &plane, const geo::WireReadoutGeom &channelMap, const detinfo::DetectorPropertiesData &dprop) {
  return {(float)(channelMap.Plane(plane).WireCoordinate(vert.position()) * channelMap.Plane(geo::PlaneID::first()).WirePitch()), (float)vert.position().X()};
}

float Vert2HitDistance(const recob::Hit &hit, const recob::Vertex &vert, const geo::WireReadoutGeom &channelMap, const detinfo::DetectorPropertiesData &dprop) {
  std::array<float, 2> vert_v = VertexVector(vert, hit.WireID(), channelMap, dprop);
  std::array<float, 2> hit_v = HitVector(hit, channelMap, dprop);
...
std::array<float, 2> HitDirection(const std::vector<art::Ptr<recob::Hit>> &hits, const recob::Vertex &vert,
               const geo::WireReadoutGeom &channelMap, const detinfo::DetectorPropertiesData &dprop) {
  if (!hits.size()) return {0., 0.};

  geo::Vector_t avg(0., 0., 0.);

  std::array<float, 2> vert_v = VertexVector(vert, hits[0]->WireID(), channelMap, dprop);
  geo::Point_t vert_p(vert_v[0], vert_v[1], 0.);

  for (const art::Ptr<recob::Hit> &h: hits) {
    std::array<float, 2> hit_v = HitVector(*h, channelMap, dprop);
...
float TrackDirectionParallel(const recob::Track &trk, const geo::PlaneID &plane,
               const geo::WireReadoutGeom &channelMap, const detinfo::DetectorPropertiesData &dprop) {
  double angleToVert = channelMap.WireAngleToVertical(channelMap.Plane(plane).View(), plane) - 0.5*::util::pi<>();
...
geo::Point_t PlaceHitAlongTrack(const recob::Track &trk, const recob::Vertex &vert, const recob::Hit &hit,
  ...
  std::array<float, 2> v_plane = VertexVector(vert, hit.WireID(), channelMap, dprop);

  // also get the 2d hit location
  std::array<float, 2> h_plane = HitVector(hit, channelMap, dprop);

  float t_dir_parallel_plane = TrackDirectionParallel(trk, hit.WireID(), channelMap, dprop);
...
geo::Point_t sbn::VertexChargeVacuum::PositionAtWires(const geo::Point_t &p, const geo::GeometryCore& geom, const spacecharge::SpaceCharge *sce) {
  ...
  if (tpc && fPositionsAreSCECorrected) return sbn::GetLocationAtWires(sce, p, geom.TPC(tpc).DriftDir());
  return p; 
}

geo::Point_t sbn::VertexChargeVacuum::PositionAbsolute(const geo::Point_t &p, const geo::GeometryCore& geom, const geo::WireReadoutGeom &channelMap, const spacecharge::SpaceCharge *sce) {
  geo::TPCID tpc = geom.FindTPCAtPosition(p);
...
void sbn::VertexChargeVacuum::produce(art::Event& evt)
{
  ...
  for (unsigned i_pfp = 0; i_pfp < pfparticles.size(); i_pfp++) {
    ...
    recob::Vertex vert_absolute(PositionAbsolute(vert.position(), *geo, channelMap, sce),
            vert.covariance(), 
            vert.chi2(),
            vert.ndof(),
            vert.ID());
    ...
    for (unsigned i_plane = 0; i_plane < 3; i_plane++) {
      ...
      for (unsigned i_hit = 0; i_hit < hits.size(); i_hit++) {
        ...
        if (hit.WireID().Plane == i_plane) {
          if (Vert2HitDistance(hit, vert_atwires, channelMap, dprop) < fHitVacuumRadius) {
	...
	sbn::VertexHit vhit;
        ...
        vhit.proj_dist_to_vertex = Vert2HitDistance(hit, vert_atwires, channelMap, dprop);
        vhit.vtxw = channelMap.Plane(hit.WireID()).WireCoordinate(vert_atwires.position());
        ...
        if (hit_sp.size()) {
          ...
          spID = sp.ID();
          spXYZ = PositionAbsolute(sp.position(), *geo, channelMap, sce);
          has_xyz = true;
        }
        // No Space-Point. If configured, see if we can look up a point along the assigned track
          ...
          if (matchingPFP) {
            ...
            if (pfptrack.size()) {
              ...
              spXYZ = PositionAbsolute(PlaceHitAlongTrack(thisTrack, vert_atwires, hit, channelMap, dprop), *geo, channelMap, sce);
              has_xyz = true;
            }
          }
        }
        ...
        if (has_xyz) {
          ...
          vhit.pitch = sbn::GetPitch(*geo, channelMap, sce, spXYZ, dir, hit.View(), hit.WireID(), fCorrectSCE, true);
