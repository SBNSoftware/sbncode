...
namespace ldm {
/**
 *  @brief  BNBPi0Gen class definiton
 */
class BNBPi0Gen : public IMesonGen
{
public:
    /**
     *  @brief  Constructor
     */
    BNBPi0Gen(fhicl::ParameterSet const &pset);
...
BNBPi0Gen::BNBPi0Gen(fhicl::ParameterSet const &pset):
  IMeVPrtlStage("BNBPi0Gen") 
{
  configure(pset);
...
//------------------------------------------------------------------------------------------------------------------------------------------
void BNBPi0Gen::configure(fhicl::ParameterSet const &pset)
{
...
std::vector<std::string> BNBPi0Gen::LoadFluxFiles() {
...
double BNBPi0Gen::GetPOT() {
  double ret = fAccumulatedPOT;
  fAccumulatedPOT = 0.;
  return ret;
}

const bsim::Dk2Nu *BNBPi0Gen::GetNextEntry() {
...
simb::MCFlux BNBPi0Gen::GetNext() {
  const bsim::Dk2Nu *flux = GetNextEntry();
  return MakeMCFlux(*flux);
}
  
simb::MCFlux BNBPi0Gen::MakeMCFlux(const bsim::Dk2Nu &dk2nu) {
  simb::MCFlux flux;
  
  flux.fFluxType = simb::kDk2Nu;
  flux.fntype    = -1; //dk2nu.decay.ntype;
...
DEFINE_ART_CLASS_TOOL(BNBPi0Gen)
