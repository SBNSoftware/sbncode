...
double HNLMakeDecay::NuMuMuWidth(double hnl_mass, double ue4, double um4, double ut4) {
    double hnl_mass_pow5 = pow(hnl_mass,5);
    double u4 = um4;//TODO: generalize, rodrigoa                                                                              
    double lep_mass = Constants::Instance().muon_mass;

    double Gfermi = Constants::Instance().Gfermi;
    double sw2   =Constants::Instance().sin2thetaW;
    double C1=(1-4*sw2+8*sw2*sw2)/4;
    double C2=(-sw2+2*sw2*sw2)   /2;

    if (hnl_mass < lep_mass * 2.) return 0.;
    long double x_e=lep_mass/hnl_mass;

    long double F1=f1(x_e);
    long double F2=f2(x_e);

    double width =u4 * (Gfermi*Gfermi*hnl_mass_pow5) / (192*M_PI*M_PI*M_PI) * ((C1+2*sw2)*F1+(C2+sw2)*F2);
...
double HNLMakeDecay::NuEEWidth(double hnl_mass, double ue4, double um4, double ut4) {
  double hnl_mass_pow5 = pow(hnl_mass,5);
  double u4 = um4;//TODO: generalize, rodrigoa
  double lep_mass = Constants::Instance().elec_mass;
  ...
  double width =u4 * (Gfermi*Gfermi*hnl_mass_pow5) / (192*M_PI*M_PI*M_PI) * (C1*F1+C2*F2);
...
double HNLMakeDecay::NuRho0Width(double hnl_mass, double ue4, double um4, double ut4) {
  ...
  long double HNLMakeDecay::L(long double x){

    //    long double func;
    // long double func2;
    //long double func3;
    
    //func=1-3*pow(x,2)-(1-pow(x,2))*sqrt(1-4*pow(x,2));
    //func=1-3*pow(x,2)-(1-pow(x,2))*sqrt(1-4*pow(x,2));
    //func2=func/(pow(x,2)*(1+sqrt(1-4*x*x)));
    //func3=log(func2);
    
    long double aux=log( (1-3*pow(x,2)-(1-pow(x,2))*sqrt(1-4*pow(x,2))) / (pow(x,2)*(1+sqrt(1-4*pow(x,2)  ) ) ) );
    return aux;
  }

  long double HNLMakeDecay::f1(long double x){
    double aux=(1-14*pow(x,2)-2*pow(x,4)-12*pow(x,6))*sqrt(1-4*pow(x,2)) + 12*pow(x,4) * (pow(x,4)-1) *L(x);
    return aux;
  }

  long double HNLMakeDecay::f2(long double x){
    double aux=4*pow(x,2)*(2+10*pow(x,2)-12*pow(x,4))*sqrt(1-4*pow(x,2))+24*pow(x,4)*(1-2*x*x+2*pow(x,4))*L(x);
    return aux;
  }
...
HNLMakeDecay::DecayFinalState HNLMakeDecay::NuDiLep(const MeVPrtlFlux &flux, bool is_muon) {
  ...
  if (!is_muon)
    {
      nue_width = NuEEWidth(flux.mass,ue4,0,0);
      numu_width = NuEEWidth(flux.mass,0,um4,0);
      nut_width = NuEEWidth(flux.mass,0,0,ut4);
    }
  if (is_muon)
    {
      nue_width = NuMuMuWidth(flux.mass,ue4,0,0);
      numu_width = NuMuMuWidth(flux.mass,0,um4,0);
      nut_width = NuMuMuWidth(flux.mass,0,0,ut4);
     }

  total_width =nue_width+numu_width+nut_width;
