...
namespace single_photon{
  ...
  void SinglePhoton::IsolationStudy(
    ...
    for(size_t t =0; t< tracks.size(); t++){
      ...
      //WARNNING  nu_slice_id is updated? Oh, tracks[0] and tracks[1] have different slide IDs (both nu slice but different nu score), need to fix this;
    ...
    for(size_t s =0; s< showers.size(); s++){
      ...
      //    if(sliceid != slice_w_bestnuID) continue;//WARNNING only deal with nu slice with best nu score for now Keng
      if(!ppfp->get_IsNuSlice()) continue;
    ...
    // IF VALID SLICE
    if(nu_slice_id >= 0){
      m_sss_num_unassociated_hits = slicehits.size()-total_shower_hits-total_track_hits;
      ...
      if(m_sss_num_unassociated_hits < 0){
        std::cout<<"ERROR!! Number of unassociated hits is negative, i.e: num_associated: "<<m_sss_num_associated_hits<<" and total slice hits: "<<slicehits.size()<<std::endl;
        exit(EXIT_FAILURE);
      }
