...
void sbn::VertexStubTracker::produce(art::Event& e)
{
  ...
  for (unsigned i_slc = 0; i_slc < slices.size(); i_slc++) {
    ...
    for (unsigned i_vhit = 0; i_vhit < vhits.size(); i_vhit++) {
      ...
      sbn::StubInfo sinfo;
      sinfo.stub = fStubBuilder.FromVertexHit(thisSlice, thisVHit, thisVHitHit, geometry, wireReadout, sce, clock_data, dprop, e, sinfo.hits, sinfo.pfp);
    ...
    for (unsigned i_mrg = 0; i_mrg < fStubMergeTools.size(); i_mrg++) {
      stubs = fStubMergeTools[i_mrg]->Merge(stubs, wireReadout, sce, clock_data, dprop);
    }
