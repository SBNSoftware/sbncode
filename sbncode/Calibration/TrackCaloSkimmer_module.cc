...
void sbn::TrackCaloSkimmer::analyze(art::Event const& e)
{
  ...
  for (const art::Ptr<raw::RawDigit> &d: rawdigitlist) {
    ...
    try {
      wids = wireReadoutGeom->ChannelToWire(d->Channel());
    }
    catch(...) {
      continue;
    }
  ...
  if (simchannels.size()) {
    ...
    id_to_ide_map = caf::PrepSimChannels(simchannels, *wireReadoutGeom);
  ...
  for (art::Ptr<recob::PFParticle> p_pfp: PFParticleList) {
    ...
    *fTrack = sbn::TrackInfo();
    ...
    // Fill the track!
    FillTrack(*trkPtr, pfp, t0, trkHits, trkHitMetas, trkHitSPs, calo, rawdigits, track_infos, wireReadoutGeom, clock_data, bt, det);
    fTrack->whicht0 = whicht0;

    FillTrackDaughterRays(*trkPtr, pfp, PFParticleList, PFParticleSPs);

    if (fFillTrackEndHits) FillTrackEndHits(geometry, wireReadoutGeom, dprop, *trkPtr, allHits, allHitSPs);

    // Fill the truth information if configured
    if (simchannels.size()) FillTrackTruth(clock_data, trkHits, mcparticles, AVs, TPCVols, id_to_ide_map, id_to_truehit_map, dprop, geometry, wireReadoutGeom);
...
sbn::TrueParticle TrueParticleInfo(const simb::MCParticle &particle,
  ...
  for (auto const &ide_pair: particle_ides) {
    const geo::WireID &w = ide_pair.first;
    unsigned c = wireReadoutGeom->PlaneWireToChannel(w);
    const sim::IDE *ide = ide_pair.second;
  ...
  for (sbn::TrueHit &h: truehits_v) {
    ...
    double xdrift = abs(h.p.x - wireReadoutGeom->Plane(geo::PlaneID(h.cryo, h.tpc, 0)).GetCenter().X());
  ...
  for (sbn::TrueHit &h: truehits_v) {
    ...
    if (closest_dist >= 0. && direction.Mag() > 1e-4) {
      ...
      geo::PlaneGeo const& planeGeo = wireReadoutGeom->Plane(plane);
      float angletovert = wireReadoutGeom->WireAngleToVertical(planeGeo.View(), plane) - 0.5*::util::pi<>();
...
void sbn::TrackCaloSkimmer::FillTrackEndHits(const geo::GeometryCore *geometry,
  ...
  float end_w = channelMap->Plane(plane_end).WireCoordinate(track.End());
  ...
  if (matches.size()) {
    ...
    for (const art::Ptr<simb::MCParticle> &p_mcp: mcparticles) {
      if (p_mcp->TrackId() == bestmatch.first) {
        ...
        fTrack->truth.p = TrueParticleInfo(*p_mcp, active_volumes, tpc_volumes, id_to_ide_map, id_to_truehit_map, dprop, geo, wireReadoutGeom);
        ...
        for (const art::Ptr<simb::MCParticle> &d_mcp: mcparticles) {
          if (d_mcp->Mother() == p_mcp->TrackId() && // correct parent
            ...
            fTrack->truth.michel = TrueParticleInfo(*d_mcp, active_volumes, tpc_volumes, id_to_ide_map, id_to_truehit_map, dprop, geo, wireReadoutGeom);
            break;
          }
        }
...
void sbn::TrackCaloSkimmer::FillTrack(const recob::Track &track, 
  ...
  for (unsigned i_hit = 0; i_hit < hits.size(); i_hit++) {
    sbn::TrackHitInfo hinfo = MakeHit(*hits[i_hit], hits[i_hit].key(), *thms[i_hit], track, sps[i_hit], calo, wireReadoutGeom, clock_data, bt_serv);
    if (hinfo.h.plane == 0) {
  ...
  for (auto const &w_pair: fWiresToSave) {
    ...
    if (rawdigits.count(wire)) {
      ...
      winfo.tpc = wire.TPC;
      winfo.channel = wireReadoutGeom->PlaneWireToChannel(wire);
      winfo.tdc0 = min_tick;
      winfo.adcs = adcs;
...
sbn::TrackHitInfo sbn::TrackCaloSkimmer::MakeHit(const recob::Hit &hit,
  ...
  hinfo.h.channel = wireReadoutGeom->PlaneWireToChannel(hit.WireID());
