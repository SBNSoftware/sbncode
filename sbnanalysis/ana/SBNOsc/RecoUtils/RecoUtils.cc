...
int SBNRecoUtils::TrueParticleID(const core::ProviderManager &manager, const art::Ptr<recob::Hit> hit, bool rollup_unsaved_ids) {
  ...
  if(track_ides.size() == 0){return -999;}
...
int SBNRecoUtils::TrueParticleIDFromTotalRecoCharge(const core::ProviderManager &manager, const std::vector<art::Ptr<recob::Hit> >& hits, bool rollup_unsaved_ids) {
...
int SBNRecoUtils::FindClosestParticle(const core::ProviderManager &manager, const TVector3& vertexposition){

  int mintrackid = -9999;
  float mindist    = 99999;

  const cheat::ParticleInventory* pi_serv = manager.GetParticleInventoryProvider();
  ...
  for (sim::ParticleList::const_iterator particleIt = particles.begin(); particleIt != particles.end(); ++particleIt){
    ...
    if((particle->Position().Vect() - vertexposition).Mag() < mindist){
      mindist = (particle->Position().Vect() - vertexposition).Mag();
      mintrackid = particle->TrackId();
    }

    if((particle->EndPosition().Vect() - vertexposition).Mag() < mindist){
      mindist = (particle->EndPosition().Vect() - vertexposition).Mag();
      mintrackid = particle->TrackId();
    }
  }
...
int SBNRecoUtils::TrueParticleIDFromTotalRecoHits(const core::ProviderManager &manager, const std::vector<art::Ptr<recob::Hit> >& hits, bool rollup_unsaved_ids) {
  ...
  const sim::ParticleList& particles = pi_serv->ParticleList();
  for (sim::ParticleList::const_iterator particleIt = particles.begin(); particleIt != particles.end(); ++particleIt){
    particle_ids.push_back(particleIt->second->TrackId());
  }
  ...
  for (std::map<int,int>::iterator trackIt = trackMap.begin(); trackIt != trackMap.end(); ++trackIt) {
    if (trackIt->second > highestCount) {
      if(std::find(particle_ids.begin(),particle_ids.end(),trackIt->first) == particle_ids.end()){continue;}
...
bool SBNRecoUtils::IsInsideTPC(const core::ProviderManager &manager, TVector3 position, double distance_buffer){
...
double SBNRecoUtils::CalculateTrackLength(const core::ProviderManager &manager, const art::Ptr<recob::Track> track){
  double length = 0;
