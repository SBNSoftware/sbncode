...
int CRUMBSTMVADriver_CCNuMu()
{
  ...
  TString outDirName("CRUMBSDataset_CCNuMu");
  gSystem->Exec("mkdir " + outDirName);
  TMVA::DataLoader *dataloader=new TMVA::DataLoader(outDirName);

  TString outfileName( outDirName + "/CRUMBSTMVA_CCNuMu.root" );
  ...
  TCut mycuts = "strstr(matchedType,\"Nu\") && !strstr(matchedType,\"DirtNu\") && matchedPurity > 0.8 && matchedCompleteness > 0.8 && ccnc == 0 && abs(nutype) == 14";
