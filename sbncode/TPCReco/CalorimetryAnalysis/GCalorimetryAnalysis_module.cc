...
sbn::GCalorimetryAnalysis::~GCalorimetryAnalysis() {
  delete fTrack;
}

sbn::GCalorimetryAnalysis::GCalorimetryAnalysis(fhicl::ParameterSet const& p)
  : EDAnalyzer{p},
    fFitExp(2),
    fFitConst(1)
{
  ...
  for (const fhicl::ParameterSet &p: selection_tool_configs) {
    fSelectionTools.push_back(art::make_tool<sbn::IGCaloSelectionTool>(p));
  }
  ...
  fTree = tfs->make<TTree>("GCalorimetryAnalyzer", "Calo Tree");
  fTree->Branch("trk", &fTrack);
}

void sbn::GCalorimetryAnalysis::analyze(art::Event const& e)
{
  unsigned evt = e.event();
  unsigned sub = e.subRun();
  unsigned run = e.run();
  std::cout << "[GCalorimetryAnalysis::analyzeEvent] Run: " << run << ", SubRun: " << sub << ", Event: "<< evt << ", Is Data: " << e.isRealData() << std::endl;
...
void sbn::GCalorimetryAnalysis::FillTrack(const recob::Track &track, 
  ...
  *fTrack = sbn::TrackInfo();
  ...
  for (const std::unique_ptr<sbn::IGCaloSelectionTool> &t: fSelectionTools) {
    if (t->Select(*fTrack)) {
      select = true;
      break;
    }
...
sbn::HitInfo sbn::GCalorimetryAnalysis::MakeHit(const recob::Hit &hit,
    unsigned hkey,
...
DEFINE_ART_MODULE(sbn::GCalorimetryAnalysis)
