...
namespace sbn::evwgh
{
  ...
  private:
    ...
    std::vector<int> fPdgs;                    //!< PDG value for particles that a given weight calculator should apply to. Note that for now this module can only handle weights for one particle species at a time.
    G4ReweighterFactory RWFactory;             //!< Base class to handle all Geant4Reweighters (right now "all" means pi+, pi-, p)
    std::vector<G4Reweighter *> theReweighter; //!< Geant4Reweighter -- this is what provides the weights
    std::vector<G4ReweightParameterMaker *> ParMaker;
    G4ReweightManager *theReweightManager;
    std::vector<std::vector<std::map<std::string, double>>> UniverseVals; //!< Vector of maps relating parameter name to value (defines parameter values that will be evaluated in universes). Each map should have one entry per parameter we are considering
    ...
    std::vector<TTree *> fOutTree_Particle; //!< Output tree for quick validations: one entry per neutrino-induced pi+, pi-, or proton
    ...
    int fPdg;                               //!< Variables for by-particle output tree
                                   ...
                                   CLHEP::HepRandomEngine &engine)
  {
    ...
    std::vector<std::string> FracsFileNames = pset.get<std::vector<std::string>>("fracsfiles");
    std::vector<std::string> XSecFileNames = pset.get<std::vector<std::string>>("xsecfiles");
    std::vector<std::vector<fhicl::ParameterSet>> FitParSets = pset.get<std::vector<std::vector<fhicl::ParameterSet>>>("parameters");
    fNsims = pset.get<int>("number_of_multisims", 0);
    fPdgs = pset.get<std::vector<int>>("pdgs_to_reweight");
    ...
    if (FracsFileNames.size() != fPdgs.size())
      throw cet::exception("Geant4WeightCalc") << "Misconfigured: Need same number of fracs files as PDGs" << std::endl;
    if (XSecFileNames.size() != fPdgs.size())
      throw cet::exception("Geant4WeightCalc") << "Misconfigured: Need same number of xsec files as PDGs" << std::endl;
    if (FitParSets.size() != fPdgs.size())
      throw cet::exception("Geant4WeightCalc") << "Misconfigured: Need same number of particle parameter sets as PDGs" << std::endl;

    UniverseVals = std::vector<std::vector<std::map<std::string, double>>>(fPdgs.size(), std::vector<std::map<std::string, double>>());

    // Prepare random generator
    fGaussRandom = new CLHEP::RandGaussQ(engine);

    if (fDebug)
      std::cout << "Setting up G4ReweightManager" << std::endl;
    theReweightManager = new G4ReweightManager(g4rw_material_pars);

    for (uint pdg_i = 0; pdg_i < fPdgs.size(); ++pdg_i)
    {
      int pdg = fPdgs[pdg_i];
      // Get input files
      TFile FracsFile(FracsFileNames[pdg_i].c_str(), "OPEN");
      TFile XSecFile(XSecFileNames[pdg_i].c_str(), "OPEN");
      
      // Configure G4Reweighter
      if (fDebug)
        std::cout << "Setting up G4ReweightParameterMaker for PDG: " << pdg << std::endl;
      ParMaker.push_back(new G4ReweightParameterMaker(FitParSets[pdg_i], true, pdg));
      theReweighter.push_back(nullptr);
      ...
      for (size_t i_mat = 0; i_mat < g4rw_material_pars.size(); i_mat++)
      {
        ...
        //{
          theReweighter[pdg_i] = RWFactory.BuildReweighter(pdg, &FracsFile, ParMaker[pdg_i]->GetFSHists(), g4rw_material_pars.at(i_mat), theReweightManager, ParMaker[pdg_i]->GetElasticHist(), false);
        //}
      }

      // Throw an exception if LAr not found
      if (theReweighter[pdg_i] == nullptr)
    ...
    if (fMakeOutputTrees)
    {
      for (uint pdg_i = 0; pdg_i < fPdgs.size(); ++pdg_i)
      {
        ...
        fOutTree_Particle.push_back(tfs->make<TTree>(Form("%s_%i", "ByParticleValidTree", pdg), ""));
        fOutTree_Particle[pdg_i]->Branch("event_num", &event_num);
        fOutTree_Particle[pdg_i]->Branch("run_num", &run_num);
        fOutTree_Particle[pdg_i]->Branch("subrun_num", &subrun_num);
        fOutTree_Particle[pdg_i]->Branch("UniverseVals", &UniverseVals[pdg_i]);
        fOutTree_Particle[pdg_i]->Branch("pdg_to_reweight", &fPdg);
        fOutTree_Particle[pdg_i]->Branch("inelastic_weight", &p_inel_weight);
        fOutTree_Particle[pdg_i]->Branch("elastic_weight", &p_elastic_weight);
        fOutTree_Particle[pdg_i]->Branch("track_length", &p_track_length);
        fOutTree_Particle[pdg_i]->Branch("PDG", &p_PDG);
        fOutTree_Particle[pdg_i]->Branch("final_proc", &p_final_proc);
        fOutTree_Particle[pdg_i]->Branch("init_momentum", &p_init_momentum);
        fOutTree_Particle[pdg_i]->Branch("final_momentum", &p_final_momentum);
        fOutTree_Particle[pdg_i]->Branch("energies_el", &p_energies_el);
        fOutTree_Particle[pdg_i]->Branch("sliceInts_el", &p_sliceInts_el);
        fOutTree_Particle[pdg_i]->Branch("energies_inel", &p_energies_inel);
        fOutTree_Particle[pdg_i]->Branch("sliceInts_inel", &p_sliceInts_inel);
        fOutTree_Particle[pdg_i]->Branch("nElasticScatters", &p_nElasticScatters);
      }
      ...
      fOutTree_MCTruth->Branch("pdgs_to_reweight", &fPdgs);
    ...
    for (uint pdg_i = 0; pdg_i < fPdgs.size(); ++pdg_i)
    {
      ...
      size_t n_parsets = FitParSets[pdg_i].size();
      ...
      {
        fhicl::ParameterSet theSet = FitParSets[pdg_i].at(i_parset);
      ...
      if (mode == "pm1sigma")
      {
        ...
        for (size_t i_parset = 0; i_parset < n_parsets; ++i_parset)
        {
          ...
          UniverseVals[pdg_i].push_back(tmp_vals_p1sigma);
          UniverseVals[pdg_i].push_back(tmp_vals_m1sigma);
        }
      }
      else if (mode == "multisim")
      {
        ...
        for (unsigned j = 0; j < fNsims; j++)
        {
          ...
          UniverseVals[pdg_i].push_back(tmp_vals);
        }
      }
      else if (mode == "cov")
      {
        ...
        for (size_t i_p = 0; i_p < FitParNames.size(); i_p++)
        {
          for (size_t j_p = 0; j_p < FitParNames.size(); j_p++)
          {
            ...
            if (!found)
            {
              ...
              if (i_p == j_p)
                Cov[i_p][j_p] = FitParSigmas.at(i_p) * FitParSigmas.at(i_p);
              else
                Cov[i_p][j_p] = 0.;
            }
          }
        ...
        for (unsigned j = 0; j < fNsims; j++)
        {
          ...
          UniverseVals[pdg_i].push_back(tmp_vals);
          std::cout << "Got " << UniverseVals[pdg_i].size() << " universes" << std::endl;
        }
      }
      else
      {
        // Anything else mode: Set parameters to user-defined nominal value
        UniverseVals[pdg_i].push_back(theNominals);
      }

      fNsims = UniverseVals[pdg_i].size();
      if (fDebug)
        std::cout << "Running mode: " << mode << ". Nsims = " << fNsims << std::endl;
    }
  }
  ...
  std::vector<float> Geant4WeightCalc::GetWeight(art::Event &e, size_t itruth)
  {
    ...
    for (size_t i = 0; i < mcparticles.size(); i++)
    {
      ...
      // We only want to record weights for the types of particle (defined by fPDGs from the fcl file), so skip other particles
      for (uint pdg_i = 0; pdg_i < fPdgs.size(); ++pdg_i)
      {
        int pdg = fPdgs[pdg_i];
        ...
        if (p_PDG == pdg)
        {
          ...
          for (size_t istep = 1; istep < nSteps; ++istep)
          {
            ...
            if (istep == p.NumberTrajectoryPoints() - 1)
              proc = EndProcess;
          ...
          for (size_t j = 0; j < weight.size(); j++)
          {
            ...
            ParMaker[pdg_i]->SetNewVals(UniverseVals[pdg_i].at(j));
            theReweighter[pdg_i]->SetNewHists(ParMaker[pdg_i]->GetFSHists());
            theReweighter[pdg_i]->SetNewElasticHists(ParMaker[pdg_i]->GetElasticHist());

            // Get the weight from the G4ReweightTraj
            float w = theReweighter[pdg_i]->GetWeight(&theTraj);
