...
void numu::MuonS2NStudy::analyze(art::Event const& ev)
{
  ...
  for (auto const &pair: amplitude_map) {
    fSignal[angle_bin][wireReadoutGeom.View(pair.first)]->Fill(pair.second);
  }

  // now get the background noise RMS

  // figure out which channels do not have any energy depositions
  std::vector<raw::ChannelID_t> allChannels = wireReadoutGeom.ChannelsInTPCs();
  ...
  for (raw::ChannelID_t c: allChannels) {
    geo::WireID wireID = wireReadoutGeom.ChannelToWire(c).at(0);
    const geo::WireGeo &wireGeo = wireReadoutGeom.Wire(wireID);
    float length = wireGeo.Length();
    // TODO: don't hardcode
    if (wireReadoutGeom.SignalType(wireID) == geo::kInduction && length < 577.3) {
      channelSet.erase(c);
    }
  }
  ...
  for (const recob::Wire &wire: *wires) {
    ...
    fNoise[wireReadoutGeom.View(wire.Channel())]->Fill(noise);
  }
