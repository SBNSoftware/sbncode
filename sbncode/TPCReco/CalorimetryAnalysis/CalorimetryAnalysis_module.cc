  ...
  for (art::Ptr<recob::PFParticle> p_pfp: PFParticleList) {
      ...
      for (const auto &pair: sc->TDCIDEMap()) {
        for (const sim::IDE &ide: pair.second) {
          if (ide.trackID == true_particle->TrackId() || fAllTrueEnergyDeposits) {
            unsigned plane_id = wireReadoutGeom.ChannelToWire(sc->Channel()).at(0).Plane;
    ...
    for (unsigned i = 0; i < particleList.size(); i++) {
      ...
      for (unsigned plane = 0; plane < 3; plane++) {
        // build the plane ID
        geo::PlaneGeo const& planeGeo = wireReadoutGeom.Plane(geo::PlaneID(0, 0, plane));
...
float calcPitch(const geo::WireReadoutGeom& channelMap, const geo::PlaneID &plane, TVector3 location, TVector3 direction, const spacecharge::SpaceCharge *SCEService=NULL) {
  geo::PlaneGeo const& planeGeo = channelMap.Plane(plane);
  float angletovert = channelMap.WireAngleToVertical(planeGeo.View(), plane) - 0.5*::util::pi<>();
            ...
            //const std::unique_ptr<art::FindManyP<simb::MCParticle, anab::BackTrackerHitMatchingData>> &assocMCPart)
{
  ...
  for (const auto &wire: allWires) {
    unsigned plane_id = wireReadoutGeom.ChannelToWire(wire->Channel()).at(0).Plane;
  ...
  if (!fData)
  {
   ...
   _backtracked_phi = trueParticle->Momentum().Phi();
    ...
    for (auto const& planeGeo: wireReadoutGeom.Iterate<geo::PlaneGeo>()) {
        ...
        for (unsigned i_traj = 0; i_traj < trueParticle->NumberTrajectoryPoints()-1; i_traj++) {
          ...
          double pitch = calcPitch(wireReadoutGeom, planeID, trueParticle->Position(i_traj).Vect(), trueParticle->Momentum(i_traj).Vect().Unit());
          double pitchSCE = calcPitch(wireReadoutGeom, planeID, trueParticle->Position(i_traj).Vect(), trueParticle->Momentum(i_traj).Vect().Unit(), spacecharge);

          // save
          int incl = wireStart < wireEnd ? 1: -1;
          for (int wire = wireStart; wire != wireEnd; wire += incl) {
            lastWire = wire;
          unsigned channel = wireReadoutGeom.PlaneWireToChannel(geo::WireID(planeID, wire));
  ...
  for (const art::Ptr<anab::Calorimetry> calo : calos)
  {
    ...
    auto const& planeGeo = wireReadoutGeom.Plane(calo->PlaneID());
