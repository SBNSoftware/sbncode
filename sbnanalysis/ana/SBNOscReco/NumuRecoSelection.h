 ...
 *         The "int" is the index into the RecoParticle daughters of the track identified as the
 *         primary track. It is set to -1 if no such track is found.
 */
  std::pair<TrackInfo,int> RecoTrackInfo(const gallery::Event &ev, const RecoParticle& vertex);
 ...
 */
  ...
  struct TrackTruthMatch {
    ...
    double completion;
    TrackTruthMatch():
      mctruth(NULL), mcparticle_id(-1), completion(-1) {}
  };

  TrackTruthMatch MatchTrack2Truth(const gallery::Event &ev, RecoParticle neutrino, int track_id);
