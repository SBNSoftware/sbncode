...
namespace sys {
  class WireModUtility{
    ...
    public:
      ...
      // geometries
      geo::TPCGeo tpcGeom() { return geometry->Cryostat(cryoID).TPC(tpcID); }

      double planeXToTick(double xPos, int planeNumber, double offset = 0) { return detPropData.ConvertXToTicks(xPos, tpcGeom().Plane(planeNumber).ID()) + offset; }

      bool planeXInWindow(double xPos, int planeNumber, double offset = 0)
      {
        double tick = planeXToTick(xPos, planeNumber, offset);
