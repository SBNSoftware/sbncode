...
namespace sbn{
/**
 *  @brief  Art tool for merging stubs across planes.
 *
 *  Uses requirements on total
 *  charge and hit timing. Re-computes location information of successful
 *  merges.
 */
...
sbn::StubInfo TwoPlaneStubMerge::MergeStubs(const sbn::StubInfo &A, const sbn::StubInfo &B,
  ...
  ret.stub.end = sbn::TwoStubEndPosition(best, othr, channelMap, sce, dprop);
  ...
  for (unsigned i_mrg = 0; i_mrg < merges.size(); i_mrg++) {
    ...
    if (thismrg.toff < fMaxMergeTOff && thismrg.qoff < fMaxMergeQOff) {
      if (!hasmerged.count(thismrg.i) && !hasmerged.count(thismrg.j)) { 
        ret.push_back(MergeStubs(stubs[thismrg.i], stubs[thismrg.j], channelMap, sce, dprop));
