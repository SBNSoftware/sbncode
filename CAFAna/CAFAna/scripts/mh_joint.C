...
std::string outputFname = "mh_sens_fd_nosyst.root";
...
void mh_joint(bool reload = false){
  ...
  std::vector<const ISyst*> systlist = GetListOfSysts(fluxsyst, normsyst);
  ...
  for(int hie = -1; hie <= +1; hie += 2){
	...
	chisqmin = TMath::Min(thischisq,chisqmin);
      ...
      if (hie > 0) gMH_NH->SetPoint(gMH_NH->GetN(),thisdcp/TMath::Pi(),TMath::Sqrt(chisqmin));
      else gMH_IH->SetPoint(gMH_IH->GetN(),thisdcp/TMath::Pi(),TMath::Sqrt(chisqmin));
    }
