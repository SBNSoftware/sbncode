...
void post_spec(const std::string anatype = numuStr)
{
  ...
  for(auto syst_pair: slists) {
    ...
    for (int j = 0; j < 1000; ++j){
      for(auto s: slist) shifts.SetShift(s, gRandom->Gaus(0, postFit[s->ShortName()]));
    ...
    for (int j = 0; j < 1000; ++j){
      for(auto s: slist) shifts2.SetShift(s, gRandom->Gaus(0, postFit[s->ShortName()]));
      hists2.push_back(p_ub->PredictSyst(&unosc, shifts2).ToTH1(uboonePOT));
    }
    ...
    for (int j = 0; j < 1000; ++j){
      for(auto s: slist) shifts3.SetShift(s, gRandom->Gaus(0, postFit[s->ShortName()]));
      hists3.push_back(p_fd->PredictSyst(&unosc, shifts3).ToTH1(icarusPOT));
    }
